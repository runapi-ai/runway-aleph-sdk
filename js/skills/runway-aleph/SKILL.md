...
- For a one-off artifact or result, use the registered `runway-aleph` service in the `runapi` CLI. If the installed command catalog does not list it, stop and report the missing service instead of inventing a command.
...
runapi runway-aleph <operation> --help
curl --fail --location https://runapi.ai/docs/api/runway-aleph/<operation>.md --output contract.md
```
...
```shell
runapi runway-aleph <operation> --async --input-file request.json > task.json
...
```shell
runapi wait "$task_id" --service runway-aleph --action <operation> > result.json
```
